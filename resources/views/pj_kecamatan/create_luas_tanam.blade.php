...
@section('title')| Tambah Luas Tanam Harian @endsection
@section('content')

<div>
    <div class="text-xl font-bold">Tambah Data Luas Tanam Harian</div>
    
    @if (!$selected_diterima)
        <div class="mt-5 mb-2">
            <a href="{{ route('kecamatan.luas_tanam') }}" class="btn btn-sm rounded-sm text-white bg-red-600 hover:bg-red-700">Kembali</a>
        </div>
        <div class="mt-2">
                ...
                <tbody>
                    @forelse ($diterima as $dt)
                        <tr>
                            ...
                            <td>{{ $dt->total_unit }}</td>
                            <td>
                                <a href="{{ route('kecamatan.luas_tanam.create', ['diterima' => Crypt::encryptString($dt->id)]) }}" class="btn btn-sm rounded-sm text-white bg-[#070] hover:bg-[#060]">Pilih</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    @else
        <div class="mt-5 mb-2">
            <a href="{{ route('kecamatan.luas_tanam.create') }}" class="btn btn-sm rounded-sm text-white bg-red-600 hover:bg-red-700">Kembali</a>
        </div>
        <form action="{{ route('kecamatan.luas_tanam.store') }}" method="POST" class="w-full">
            @csrf
            ...
            <div class="flex flex-col py-1">
                <label for="luas_tanam" class="text-semibold">Total Luas Tanam (Ha)<span class="text-red-500">*</span></label>
                <input type="number" step="0.0001" min="0" max="{{ $selected_diterima->luas_lahan }}" name="luas_tanam" id="luas_tanam" class="py-1 px-2 w-98 rounded-sm border-1 border-gray-400" required>
            </div>
